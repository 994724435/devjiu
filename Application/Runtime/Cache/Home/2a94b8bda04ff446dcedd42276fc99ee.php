...
<head>
	...
	<script type="text/javascript" src="/dev/devjiu/Public/Home/js/index.js?v=2"></script>
	<link rel="stylesheet" type="text/css" href="/dev/devjiu/Public/Home/css/aui.min.css">
	<link rel="stylesheet" type="text/css" href="/dev/devjiu/Public/Home/css/style.css?v=1">
	<title>酒模式</title>
</head>
<body class="indexBg">
<ul class="lands">
	<?php if(is_array($land)): foreach($land as $key=>$v): ?><li class="land land<?php echo ($v["num"]); ?>" style="background: url(/dev/devjiu/Public/Home/img/p_2.png) no-repeat;" data-land="<?php echo ($v["num"]); ?>">
		<?php if($v["state"] == 1): ?><img src="/dev/devjiu/Public/Home/img/p_3.png" alt="" class="jiuTong"><?php endif; ?>
	...
	<div class="maskContent" id="shopMask">
		...
		<?php if(is_array($productlist)): foreach($productlist as $key=>$v): ?><ul class="maskList">
			<li>
				...
				<span class="buySpan" data-money="<?php echo ($v["price"]); ?>" data-id="<?php echo ($v["id"]); ?>">购买</span>
			</li>
		...
		<form action="/dev/devjiu/index.php/Home/Index/buyProduct" method="post"  enctype="multipart/form-data">
			...
			<ul>
				<li class="main_com_li">
					...
					<input type="text" name="kucun" disabled="disabled" value="500">
				</li>
				<li class="main_com_li">
		...
		<form action="/dev/devjiu/index.php/Home/Index/buyProduct"  method="post"  enctype="multipart/form-data">
			...
			<ul>
				...
				<li class="main_com_li">
					...
					<input type="text" name="yunfei" value="5" disabled="disabled">
				</li>
				<li class="main_com_li">
