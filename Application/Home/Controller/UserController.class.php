...
class UserController extends CommonController{
    ...
    public function rechargeEnd(){
        $incomelog = M("incomelog");
        $log = $incomelog->where(array('id'=>$_GET['id']))->select();
        $this->assign('log',explode(',',$log[0]['cont']));
        $this->display();
    }
    ...
    public function buys(){
        ...
        if($log[0]['userid'] == session('uid')){
            echo "<script>alert('自己不能购买自己的挂卖');";
        ...
        //生成新纪录
        $commitid = date("YmdHis").rand(1000,9999);
        $log[0]['userid'] = session('uid');
        $log[0]['commitid'] = $commitid;
        $log[0]['orderid'] = 2;
        $log[0]['state'] = 4;
        unset($log[0]['id']);
        $incomelog->add($log[0]);

        $incomelog->where(array('id'=>$_GET['id']))->save(array('state'=>4,'orderid'=>1,'commitid'=>$commitid));
    ...
    public function rechargeInfo(){
        $incomelog = M("incomelog");
        $log = $incomelog->order('id DESC')->where(array('type'=>12,'state'=>0))->select();
        //查询待处理的结果
        $map['state']  = array('gt',3);
        $map['type']  = 12;
        $map['userid']  = session('uid');
        $nextlog =$incomelog->where($map)->select();

        $this->assign('nextlog',$nextlog);
        $this->assign('log',$log);
        $this->display();
    }
    ...
    public function recharge(){
        ...
        if($_POST['money'] > 0 ){
            if(!$_POST['weixin'] || !$_POST['tel']){
                echo "<script>alert('请将信息填写完整');";
                echo "window.location.href='".__ROOT__."/index.php/Home/User/recharge';";
                echo "</script>";
                exit;
            }

            if($_POST['money'] % 100 != 0 ){
                echo "<script>alert('需要挂卖100的倍数');";
                echo "window.location.href='".__ROOT__."/index.php/Home/User/recharge';";
                echo "</script>";
                exit;
            }
            ...
            $lilv =bcmul($_POST['money'],0.05,2);
            $allneed = bcadd($lilv,$_POST['money'],2);

            $userinfo =M('menber')->where(array('uid'=>session('uid')))->select();

            if($userinfo[0]['chargebag'] < $allneed){
            ...
            $data['orderid'] = 0;
            $data['cont'] = $_POST['weixin'].",".$_POST['tel'];
            $data['username'] = $userinfo[0]['name'];
            $logid= $order->add($data);

            $lefts = bcsub($userinfo[0]['chargebag'],$_POST['money'],2);
            $left = bcsub($lefts,$lilv,2);
            M('menber')->where(array('uid'=>session('uid')))->save(array('chargebag'=>$left));

            echo "<script>alert('售卖成功');";
            echo "window.location.href='".__ROOT__."/index.php/Home/User/recharge';";
            echo "</script>";
            exit;
        }
